...
def enrich_with_country(fraud_df: pd.DataFrame, ip_df: pd.DataFrame) -> pd.DataFrame:
    ...
    logger.info(
        "Geolocation: %d/%d matched (%.1f%%), %d unique countries",
        n_ok,
        len(result),
        n_ok / len(result) * 100,
...
def preprocess_fraud_data(fraud_path: str, ip_path: str) -> pd.DataFrame:
    ...
    logger.info(
        "Removed %d duplicates (%d remaining)", n_before - len(fraud), len(fraud)
    )
    ...
    fraud["transaction_velocity_1h"] = fraud.groupby("user_id", group_keys=False).apply(
        lambda g: count_velocity(g, 3600)
    )
    logger.info("Computing 24h velocity ...")
    fraud["transaction_velocity_24h"] = fraud.groupby(
        "user_id", group_keys=False
    ).apply(lambda g: count_velocity(g, 86400))
    logger.info("Encoding categoricals ...")
    fraud = pd.get_dummies(
        fraud, columns=["source", "browser", "sex"], drop_first=False, dtype=int
    )
    le = LabelEncoder()
...
def preprocess_creditcard(cc_path: str) -> pd.DataFrame:
    ...
    logger.info(
        "Class dist:\n%s", cc["Class"].value_counts(normalize=True).round(4).to_string()
    )
    return cc
...
def apply_smote(X_train, y_train, random_state=RANDOM_STATE):
    ...
    logger.info("SMOTE before: %s", dict(zip(*np.unique(y_train, return_counts=True))))
    X_res, y_res = SMOTE(random_state=random_state).fit_resample(X_train, y_train)
    logger.info("SMOTE after:  %s", dict(zip(*np.unique(y_res, return_counts=True))))
    return X_res, y_res


def apply_undersampling(
    X_train, y_train, sampling_strategy=0.5, random_state=RANDOM_STATE
):
    """Apply RandomUnderSampler to training set only."""
    logger.info(
        "UnderSampler before: %s", dict(zip(*np.unique(y_train, return_counts=True)))
    )
    X_res, y_res = RandomUnderSampler(
        sampling_strategy=sampling_strategy, random_state=random_state
    ).fit_resample(X_train, y_train)
    logger.info(
        "UnderSampler after:  %s", dict(zip(*np.unique(y_res, return_counts=True)))
    )
    return X_res, y_res
